...
\begin{exampleblock}{Обозначение}
	$\boxed{\#_iX:= \{ \varphi \mid K_i \varphi \in X \}}$
\end{exampleblock}
...
\fh \fw{X} \; X \in W^\Phi & $\rhd \; X \sim^\Phi_i X
	\Leftrightarrow \rhd \; \forall \varphi \in \Phi (K_i \varphi \in X \Rightarrow \varphi \in X)$ \\
%2%
	\fa \fh \fw{\varphi}\in \Phi: K_i \varphi \in X & $\rhd \; \varphi \in X$ \\ 
%3%
...
\begin{itemize}
\item[БИ] $\varphi'=p$
\item[ПИ] $\forall \psi \in Sub(\varphi'): \psi \in X \iff M^\Phi, X \models \psi$
\item[ШИ]
\begin{itemize}
\item[Сл.1] $\varphi'= \neg \varphi$	
...
\frame{\frametitle{Сл. 3. $\varphi'= K_i \varphi$	}}
