...
$cl_0(\varphi) \subset cl_1(\varphi) \subset cl_2 (\varphi) \subset cl(\varphi)$.
\begin{description}
\item[$cl_0(\varphi)$] – наименьшее множество, замкнутое по следующим правилам:
\begin{enumerate}
\item $\varphi \in cl_0(\varphi)$
\item если $\psi \in cl_0(\varphi)$, то $Sub(\psi) \subseteq cl_0(\varphi)$
\item если $C_G \psi \in cl_0(\varphi)$, то $\{K_i C_G \psi \mid i \in G \} \subseteq cl_0(\varphi)$	
\end{enumerate}
\item[$cl_1(\varphi)$]$:= cl_0(\varphi) \cup \neg cl_0(\varphi)$	 
\item[$cl_2(\varphi)$]$:= cl_1(\varphi) \cup \{K_i K_i \psi \mid K_i \psi \in cl_1(\varphi) \} \cup \{K_i \neg K_i \psi \mid \neg K_i \psi \in cl_1(\varphi) \}$	
\item[$cl(\varphi)$]$:=cl_2(\varphi) \cup \neg cl_2(\varphi)$
\end{description}
\end{block}
}
...
%\begin{itemize}
%\item[$\varphi$] $= \neg p \wedge C_{ab}q$ 
%\item[$cl_0(\varphi)$] $= \{\neg p \wedge C_{ab}q,\neg p, p,   C_{ab}q, K_a C_{ab}q, K_bC_{ab}q\} $ 	
%\item[$cl_1(\varphi)$]$ = \{\neg p \wedge C_{ab} q, \alert{\neg (\neg p \wedge C_{ab} q)}, \neg p, p, C_{ab}q, \alert{\neg C_{ab}q}, K_a C_{ab}q, \alert{\neg K_a C_{ab}q}, K_bC_{ab}q, \alert{\neg K_bC_{ab}q} \} $
%\item[$cl_2(\varphi)$] $= \{ \neg p \wedge C_{ab} q, \neg (\neg p \wedge C_{ab} q), \neg p, p, C_{ab}q, \neg C_{ab}q, K_a C_{ab}q, \alert{K_a} K_a C_{ab}q, \neg K_a C_{ab}q, \alert{K_a} \neg K_a C_{ab}q, $ \\
%$K_b C_{ab}q, \alert{K_b} K_b C_{ab}q, \neg K_b C_{ab}q, \alert{K_b} \neg K_b C_{ab}q \}$ 
%\item[$cl(\varphi)$] $= \{ 
%\neg p \wedge C_{ab} q,
%\neg (\neg p \wedge C_{ab} q),
%\neg p,
...
%C_{ab}q,
%\neg C_{ab}q,
% K_a C_{ab}q, K_a K_a C_{ab}q, \alert{\neg K_a K_a C_{ab}q},  \neg K_a C_{ab}q,$ \\ $K_a \neg K_a C_{ab}q, \alert{\neg K_a \neg K_a C_{ab}q}, K_b C_{ab}q, K_b K_b C_{ab}q, \alert{\neg K_b K_b C_{ab}q},   \neg K_b C_{ab}q, K_b \neg K_b C_{ab}q,$\\ $\alert{\neg K_b \neg K_b C_{ab}q} \}$ 
%\end{itemize}
%
%\end{exampleblock}
...
\frame{
\begin{exampleblock}{Пример $cl_0(\varphi) \subset cl_1(\varphi) \subset cl_2 (\varphi) \subset cl(\varphi)$}
\centering 
\scalebox{0.8}{
\begin{tabular}{|c|c|c|c|c|}
\hline
$\varphi$               & $cl_0(\varphi)$         & $cl_1(\varphi)$                  & $cl_2(\varphi)$                 & $cl(\varphi)$                   \\ \hline
