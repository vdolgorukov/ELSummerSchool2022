...
\begin{block}{Языки ($i \in Ag, G\subseteq Ag$)}
\begin{tabular}{ll}
$(EL)$   &    $ p \mid \neg \varphi \mid (\varphi_1 \wedge \varphi_2) \mid K_i \varphi$ \\
$(ELE)$  &   $ p \mid \neg \varphi \mid (\varphi_1 \wedge \varphi_2) \mid K_i \varphi \mid E_G \varphi$\\
$(ELD)$ &   $ p \mid \neg \varphi \mid (\varphi_1 \wedge \varphi_2) \mid K_i \varphi \mid D_G \varphi$\\
$(ELC)$	&    $ p \mid \neg \varphi \mid (\varphi_1 \wedge \varphi_2) \mid K_i \varphi \mid C_G \varphi$
\end{tabular} 
\end{block}}
...
\begin{block}{Определение}
Модальный язык $L_2$ является не менее выразительным, чем модальный язык $L_1$ в классе моделей Крипке $C$ (обозначение: <<$L_1 \sqsubseteq_{C} L_2$>>) е.т.е.  
\begin{center}
...
\begin{itemize}
\item $L_1 \sqsubseteq L_2:= L_1 \sqsubseteq_{All} L_2$, где $All$ – класс всех моделей Крипке 
\begin{multicols}{2}
\item $L_1 \equiv_{C} L_2 := L_1 \sqsubseteq_{C} L_2 \text{ и } L_2  \sqsubseteq_{C} L_1 $
\item $L_1 \sqsubset_{C} L_2 := L_1 \sqsubseteq_{C} L_2 \text{ и } L_2  \not \sqsubseteq_{C} L_1 $
\item $L_1 \equiv  L_2 := L_1 \sqsubseteq  L_2 \text{ и } L_2  \sqsubseteq  L_1 $
\item $L_1 \sqsubset L_2 := L_1 \sqsubseteq L_2 \text{ и } L_2  \not \sqsubseteq L_1 $
\end{multicols}
\end{itemize}
\end{block}
}
...
\begin{exampleblock}{Утверждения.}
\begin{enumerate}
\item $EL \equiv EL+E$	
\item $EL \sqsubset EL+D$	
\item $EL \sqsubset EL+C$	
\end{enumerate}
\end{exampleblock}
...
\frame{\frametitle{$EL \sqsubset ELD$}
Пример
}
...
\frame{\frametitle{$EL \sqsubset_{S5} ELC$}
$EL \sqsubseteq ELC$ – очевидно. Следовательно $EL \sqsubseteq_{S5} ELC$. Докажем, что $ELC \not \sqsubseteq_{S5} EL$. 
}
...
$EL=ELE$, но $ELE$ более лаконичный (succinct).
